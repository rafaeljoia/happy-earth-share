...
function renderTitle(titulo: string): ReactNode {
  const parts = titulo.split(/(acolhimento\.?)/i);
  if (parts.length === 1) return titulo;

  return parts.map((part, i) =>
    /^acolhimento\.?$/i.test(part) ? (
      <em key={i} className="italic text-sun">
        {part}
      </em>
    ) : (
    ...
    >
      ...
      <div className="relative z-10 mx-auto flex min-h-[88vh] max-w-6xl items-center px-4 py-24 md:min-h-[92vh] md:py-28">
        <div className="reveal max-w-2xl">
          <p className="font-display text-lg font-semibold tracking-tight text-sun md:text-xl">
            ONG Viver Feliz
          </p>
          <h1 className="mt-4 text-[2.15rem] font-semibold leading-[1.12] tracking-tight text-white md:text-5xl lg:text-[3.35rem]">
            {renderTitle(site.hero.titulo)}
          </h1>
