...
export function Comments() {
  ...
  return (
    <section id="depoimentos" className="mx-auto max-w-4xl px-4 py-16">
      <div className="text-center">
        <div className="chip bg-leaf-soft text-secondary-foreground">Depoimentos</div>
        <h2 className="mt-3 text-3xl font-bold md:text-4xl">Recados da comunidade</h2>
        ...
        {loading ? (
          ...
          <form onSubmit={submit} className="space-y-3">
            ...
            <textarea
              ...
              rows={3}
              className="w-full resize-none rounded-2xl border border-input bg-background px-4 py-3 text-sm outline-none focus:border-ring"
              required
            />
            <div className="flex items-center justify-between">
              <span className="text-xs text-muted-foreground">{text.length}/1000</span>
              <button type="submit" disabled={sending || !text.trim()} className="btn-primary text-sm !py-2 !px-4">
                <Send size={16} /> {sending ? "Enviando..." : "Publicar"}
              </button>
            </div>
          </form>
        ) : (
          <div className="flex flex-col items-center gap-3 py-6 text-center">
            ...
            <button onClick={handleGoogle} className="btn-leaf">
              <LogIn size={18} /> Fazer login com o Google para comentar
            </button>
          </div>
        )}
      ...
      <ul className="mt-8 space-y-4">
        ...
        {comments.map((c) => (
          <li key={c.id} className="card-soft flex gap-3 p-4">
            {c.author_avatar ? (
              <img src={c.author_avatar} alt="" className="h-10 w-10 shrink-0 rounded-full" />
            ) : (
              <div className="h-10 w-10 shrink-0 rounded-full bg-primary/40" />
            )}
